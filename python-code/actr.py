...
def get_chunk_creation_time(chunk):
    return current_connection.evaluate_single("chunk-creation-time", chunk)
...
def set_similarities(*prarms):
    return current_connection.evaluate_single("set-similarities", *prarms)

def similarity(*prarms):
    return current_connection.evaluate_single("similarity", *prarms)

def chunk_filled_slots_list(*prarms):
    return current_connection.evaluate_single("chunk-filled-slots-list", *prarms)

def merge_dm_chunks(*prarms):
    # (merge-dm-fct (list (chunk-spec-to-chunk-def (chunk-name-to-chunk-spec 'd))))
    # chunk_spec = current_connection.evaluate_single("chunk-name-to-chunk-spec", *prarms)
    # chunk-def = current_connection.evaluate_single("chunk-spec-to-chunk-def", chunk_spec)
    return current_connection.evaluate_single("merge-dm-chunks", *prarms)

def reset_declarative_finsts(*prarms):
    return current_connection.evaluate_single("reset-declarative-finsts", *prarms)

def remove_visual_finsts(*prarms):
    return current_connection.evaluate_single("remove-visual-finsts", *prarms)